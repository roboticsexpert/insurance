...
src/
  ...
  lib/reference.ts         فهرست گزینه‌های فرم استعلام، از `GET /catalog/reference/:key`
  lib/jalali.ts            تبدیل جلالی ⇄ میلادی (کپی از `apps/web`)
...
- **گزینه‌های فرم استعلام از `reference`های API می‌آیند، نه از CMS** — مقدارشان باید
  عیناً همان چیزی باشد که ویزارد می‌فرستد. اگر API جواب ندهد، همان فیلد ساخته نمی‌شود و
  ویزارد می‌پرسدش؛ صفحه نمی‌شکند. جدولش در `LANDING-PAGES.md`.
