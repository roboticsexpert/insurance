...
| `insurerStrip` | لوگوی شرکت‌ها — فقط `active`ها |
| `offerPreview` | پیش‌نمایش مقایسه نرخ، همیشه با برچسب «نرخ نمونه» |
| `coverageList` | چه چیزی پوشش دارد / ندارد / اختیاری است |
| `priceFactors` | قیمت به چه چیزهایی بستگی دارد |
...
- شرکت‌های بیمه با وضعیت **«در حال مذاکره»** ساخته می‌شوند و `insurerStrip` فقط `active`ها را
  نشان می‌دهد. تا قرارداد امضا نشده، لوگو روی سایت ادعای همکاری است.
- `offerPreview` **همیشه** برچسب «نرخ نمونه» دارد؛ نرخ زنده‌ای در کار نیست.
...
- استقرار هنوز انجام نشده. `apps/api` و `apps/web` روی Railway‌اند و این هم احتمالاً همان‌جا
  می‌رود (Postgres همان‌جاست)، اما تصمیمی گرفته نشده و Dockerfile قالب حذف شده است.
