/**
 * تبدیل جلالی ⇄ میلادی، با `Intl` به‌عنوان تنها مرجع.
 *
 * عیناً از `apps/web/src/lib/jalali.ts` کپی شده، به همان دلیلی که `lib/fa.ts` کپی شده:
 * چند تابع ثابت ارزان‌تر از یک پکیج مشترک و یک مرحله بیلد بین‌پکیجی است. اگر آنجا
 * عوض شد، اینجا هم عوضش کنید.
 *
